...
const Header2 = props => {
  ...
  }  uppercase hover:text-blue `
  
  const fillColor = scroll ? "black" : "white"
  return (
    <header
      className={`headerWrapper ${position} z-50 w-full  ${
        scroll ? " sm:bg-white" : ""
      }`}
    >
      <nav className={`flex justify-between sm:px-1/20 ${props.menuState ? "bg-space" : ""} sm:bg-transparent`}>
        {/* <div className=""> */}
          <Link to="/" className=" hidden sm:block w-28 md:w-32 //lg:w-48 ">
        ...
        {/* </div> */}
        <div
          className={`rightDiv  hidden flex sm:flex flex-col justify-center items-center sm:flex-row w-screen sm:w-screen /bg-white sm:bg-transparent h-screen sm:h-auto sm:w-max flex-wrap sm:flex-nowrap  md:justify-between md:w-3/5  pt-10 xxsm:pt-1/5 xsm:pt-1/10 sm:pt-0  ${menuActive}`}
        >
          <Link
            onClick={props.toggleMenu}
            to="/#mission"
